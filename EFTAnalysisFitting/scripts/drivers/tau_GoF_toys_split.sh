...
OUTDIR=$SCRIPT_DIR'/../../unblind/output/leave_one_out/'
echo "LOGDIR=${LOGDIR}"
...
for ((i=0; i<length; i++)); do
    ...
    echo "combine -M GoodnessOfFit /uscms_data/d3/ckampa/EFTAnalysis/EFTAnalysisFitting/unblind/workspaces/leave_one_out/VVV.all_combined_LOO_not_tau.dim6_All.workspace.vCONFIG_VERSIONS_NDIM.root --algo=saturated --redefineSignalPOIs k_cW --freezeNuisanceGroups nosyst --freezeParameters r,k_cHl3,k_cHbox,k_cHDD,k_cHq1,k_cHq3,k_cHW,k_cHWB,k_cll1,k_cHB,k_cHu,k_cHd --setParameters r=1 --setParameterRanges k_cW=-2,2 --verbose 1 -n _Toys1000_${i}.all_tau.GoF.vCONFIG_VERSIONS_NDIM --toys 100 --seed=${SEEDS[i]} > ${LOGDIR}GoF_tau_comb_toys_1000_${i}_SEED_${SEEDS[i]}.txt 2>&1 &"
