...
while read channel version; do
    ...
    for WC in "${WCloop[@]}"; do
        ...
        echo "python 1D_scan/run_combine_1D.py -c $channel -w ${WC} -t ${T} -s _All -p 0.01 -pc 0.05 -U y -a ${AS} -T y ${V} > ${LOGDIR}run_combine_All_${A}_${CHF}_${T}_${WC}_${VF}.txt 2>&1 &"
        python 1D_scan/run_combine_1D.py -c $channel -w ${WC} -t ${T} -s _All -p 0.01 -pc 0.05 -U y -a ${AS} -T y ${V} > ${LOGDIR}run_combine_All_${A}_${CHF}_${T}_${WC}_${VF}.txt 2>&1 &
    done
done < <(python -c "
import sys
sys.path.append('${VDICT_DIR}')
