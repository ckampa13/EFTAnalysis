...
def make_summary_table(results_dict, dim='dim6', order_sens=True, tex_file=None):
    ...
    if order_sens:
        ...
        # if dim == 'dim6':
        table += r"\topcaption{Summary of the 95\% \CL bounds and measurements on the dim-6 Wilson coefficients. We consider the case of a single varying Wilson coefficient (``Freeze Other WCs'') as well as the case when the other Wilson coefficients are profiled over (``Profile Other WCs''). The Wilson coefficients are ordered by increasing confidence interval width. \label{tab:old_limit_summary_"+f"{dim}"+"_with_profile}}" + "\n"
        # else:
        ...
        # table += r"\label{tab:limit_summary_"+f"{dim}"+"_alt_order_with_profile}" + "\n"
        #if dim == 'dim6':
        table += r"\topcaption{Summary of the 95\% \CL bounds and measurements on the dim-6 Wilson coefficients. We consider the case of a single varying Wilson coefficient (\"Freeze Other WCs\") as well as the case when the other Wilson coefficients are profiled over (``Profile Other WCs''). \label{tab:old_limit_summary_"+f"{dim}"+"_alt_order_with_profile}}" + "\n"
        # else:
    ...
    #table += r"\end{table}" + "\n"
    table += r"\begin{tabular}{r|c|c|c|c|c|c}" + "\n"
    table += r"\hline" + "\n"
    table += r"& \multicolumn{3}{p{11cm}|}{\centering Freeze Other WCs} & \multicolumn{3}{p{11cm}}{\centering Profile Other WCs} \\" + "\n"
    table += r"\hline" + "\n"
    ...
    table += r"Wilson & \multicolumn{2}{p{7cm}|}{\centering 95\% \CL Bounds $[$TeV$^{-2}]$} & \multicolumn{1}{p{4cm}|}{\centering Measurement $[$TeV$^{-2}]$} & \multicolumn{2}{p{7cm}|}{\centering 95\% \CL Bounds $[$TeV$^{-2}]$} & \multicolumn{1}{p{4cm}}{\centering Measurement $[$TeV$^{-2}]$} \\" + "\n"
    table += r"coefficient & \multicolumn{1}{p{3.5cm}|}{\centering Observed} & Expected & Observed & \multicolumn{1}{p{3.5cm}|}{\centering Observed} & Expected & Observed \\" + "\n"
    wc_suff = r'/\Lambda^{2}$'
    # else:
...
if __name__=='__main__':
    ...
    # for dim in ['dim6', 'dim8_partial']:
        ...
        #_ = print_summary_table_lim_sorted(ddir, dim, dim_dict, order_sens=order_sens, return_dict=True)
    ...
    for dim in dims_list:
        if order_sens:
            tex_file = plotdir+f'limit_summary_{dim}_with_profile.tex'
        else:
            tex_file = plotdir+f'limit_summary_{dim}_alt_order_with_profile.tex'
        make_summary_table(results_dict_dim[dim], dim=dim, order_sens=order_sens, tex_file=tex_file)
