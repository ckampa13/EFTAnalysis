...
def run_impact_full_analysis_leave_one_out(channel_leave_out, dim, WC, limit_val, ScanType, Asimov,
    ...
    plot_dir = os.path.join(dcdir, 'plots', 'leave_one_out', scan_dir)
    os.chdir(outdir)
    # add any frozen WC
    if ScanType == '_1D':
        WCs_freeze = []
...
if __name__=='__main__':
    ...
    parser.add_argument('-c', '--Channel',
                        help='Which channel to leave out? ["all_tau" (default, remove tau channels), "not_tau" (combine tau channels), "0Lepton_2FJ", "0Lepton_3FJ", "2Lepton_OS", "2Lepton_SS"]')
    ...
    # WCs = WC_ALL
    # if WC in WCs:
    run_impact_full_analysis_leave_one_out(channel, dim, WC=WC,
