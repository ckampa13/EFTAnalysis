...
if __name__=='__main__':
    ...
    datacard_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)),
                                '..')
    output_base = 'output'
    output_sub = 'single_channel_single_bin'
    ...
    _ = subprocess.run(f'combine -M MultiDimFit {workspacefile} --algo=grid --points {poi_dict["steps"]} --alignEdges 1 {asi_str} --redefineSignalPOIs k_cG --freezeParameters r --setParameters r=1 --setParameterRanges k_cG={poi_dict["LL"]},{poi_dict["UL"]} --verbose -1 -n _{cardfile_base}', stdout=stdout, shell=True)
