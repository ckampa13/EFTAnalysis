...
int ReadWWZ_Delphes(std::string infile, std::string outfile){
  ...
  for (int i=0; i<nEvents ; ++i)
  {
     ...
     for (unsigned int j=0; j<GenParticle_PDGId->size(); ++j)
     {
       ...
       //std::cout << "genpart.motherone = " << genpart.motherone << std::endl;
     ...
     */
     ...
     if(genmuons.size() >= 4) std::cout << "genmuons.size() = " << genmuons.size() << std::endl;
     //if(genmuons.size() >= 3)
     //{ 
  ...
  if(genmuons.size()>=3) h_MET->Fill(MET);
  ...
  if(genmuons.size()>=4) h_nJets->Fill(Jet_vector.size());
  ...
  if(genmuons.size()>=3) h_ST->Fill(ST);
  h_HTb->Fill(HTb);
