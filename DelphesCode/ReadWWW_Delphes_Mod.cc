...
int ReadWWW_Delphes(std::string infile, std::string outfile){
  ...
  TH1F *h_Mjj_closest_fiducialRegion = new TH1F("h_Mjj_closest_fiducialRegion", "h_Mjj; Mjj of the two closest jets in the fiducial region; Events", 1500, 0, 150); h_Mjj_closest_fiducialRegion->Sumw2();
  ...
  for (unsigned int i = 0; i < Jet_vector.size(); ++i)
  {
   ...
   if(muons.size()==2 and muons.at(0).pT > 25.0 and muons.at(1).pT > 25.0 and muons.at(0).charge*muons.at(1).charge==1)//exactly 2 muons
   {
     ...
     {
       if(Mjj > 0.0) h_Mjj_closest_fiducialRegion->Fill(Mjj);
  ...
  h_Mjj_closest->Write();
  h_Mjj_closest_fiducialRegion->Write();
  h_nJets->Write();
  h_HTb->Write();
  h_nbJets->Write();
