...
void makePlots_BW()
{
  ...
  double lumi = 137.0;
  double onshell_crosssection =  0.0740657589237*1000.0;//0.132115222634*1000.0;//0.0777669628521*1000.0;//0.482086643454*1000.0;
  double onshell_crosssection_int = 0.00129278089682*1000.0;//0.0451520294276*1000.0;//0.00113380967846*1000.0;//0.0113714476406*1000.0; 
  double onshell_crosssection_bsm = 0.000451387804781*1000.0;//0.03575804*1000.0;//0.00882216245354*1000.0;//0.891326097209*1000.0; //0.0068372*1000.0;//0.007216*1000.0
  double onshell_crosssection_sm = 0.073458606*1000.0; 

  TFile* filedim6All = new TFile("test_unweighted_events_WWW_dim6_cHW_0p5_All.root");
  TH1F *hdim6All = (TH1F*)filedim6All->Get("M_www");
  hdim6All->SetTitle("");
  hdim6All->SetLineColor(kBlack);
  hdim6All->SetLineWidth(2);
  hdim6All->Rebin(10);
  hdim6All->GetXaxis()->SetRangeUser(0.0, 10000.0);
  ...
  TFile* filedim6Interference = new TFile("test_unweighted_events_WWW_dim6_cHW_0p5_Interference.root");
  ...
  hdim6Interference->GetXaxis()->SetRangeUser(0.0, 10000.0);
  ...
  TFile* filedim6BSM = new TFile("test_unweighted_events_WWW_dim6_cHW_0p5_BSM.root");
  TH1F *hdim6BSM = (TH1F*)filedim6BSM->Get("M_www");
  hdim6BSM->SetLineColor(kBlue);
  hdim6BSM->SetLineWidth(2);
  hdim6BSM->Rebin(10);
  hdim6BSM->GetXaxis()->SetRangeUser(0.0, 10000.0);
  ...
  hdim6SM->Rebin(10);
  hdim6SM->GetXaxis()->SetRangeUser(0.0, 10000.0);
  ...
  leg1->SetFillStyle(0);
  leg1->SetHeader("C_{HW}/#Lambda^{2} = 0.5 TeV^{-2}");
  ...
  c1.SaveAs("WWW_dim6_cHW_0p5_simpleComparison.png");
  c1.SaveAs("WWW_dim6_cHW_0p5_simpleComparison.pdf");
}
